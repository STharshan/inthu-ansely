export default function SideBySideComparison() {
  ...
  const platforms = [
    ...
    { key: "godaddy", label: "GoDaddy", highlight: false },
  ];
    ...
    <div className="flex gap-0.5">
      {[...Array(5)].map((_, i) => (
        <svg
          ...
          viewBox="0 0 20 20"
          fill="currentColor"
          className={`h-4 w-4 ${i < rating ? "text-[#0045EF]" : "text-gray-300 dark:text-gray-700"}`}
        >
    ...
    <section className="py-16 md:py-24 bg-white dark:bg-black transition-colors">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 md:mb-12">
          ...
          <p className="mt-4 max-w-2xl mx-auto text-sm sm:text-base text-gray-700 dark:text-gray-300">
            See how React stacks up against traditional website builders across key metrics.
          </p>
        </div>

        {/* Desktop Table — hidden on mobile */}
        <div className="hidden md:block rounded-2xl border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 overflow-hidden">
          <table className="w-full text-left">
            <thead className="border-b border-gray-300 dark:border-gray-700">
              <tr>
                <th className="px-6 py-4 text-black dark:text-white font-semibold">Feature</th>
                {platforms.map((p) => (
                  <th
                    key={p.key}
                    className={`px-6 py-4 font-semibold ${p.highlight ? "text-[#0045EF]" : "text-gray-700 dark:text-gray-300"}`}
                  >
                    {p.label}
                  </th>
                ))}
            ...
            <tbody>
              {data.map((row, i) => (
                ...
                >
                  <td className="px-6 py-5 text-black dark:text-white font-medium">{row.label}</td>
                  {platforms.map((p) => (
                    <td key={p.key} className="px-6 py-5">
                      <Stars rating={row[p.key]} />
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <div className="md:hidden space-y-4">
          {data.map((row, i) => (
            <div
              key={i}
              className="rounded-2xl border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 overflow-hidden"
            >
              {/* Card header */}
              <div className="px-4 py-3 border-b border-gray-300 dark:border-gray-700 bg-gray-200 dark:bg-gray-700">
                <span className="text-black dark:text-white font-semibold text-sm">{row.label}</span>
              </div>
              {/* Platform rows */}
              <div className="divide-y divide-gray-300 dark:divide-gray-700">
                {platforms.map((p) => (
                  <div key={p.key} className="flex items-center justify-between px-4 py-3">
                    <span
                      className={`text-sm font-medium ${p.highlight ? "text-[#0045EF]" : "text-gray-700 dark:text-gray-300"}`}
                    >
                      {p.label}
                    </span>
                    <Stars rating={row[p.key]} />
        ...
        {/* Tablet: horizontal scroll fallback for very narrow tablets */}
        <style>{`
