  ...
  return (
    <>
      <section
        ref={sectionRef}
        id="services"
        className={`relative px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24 overflow-hidden ${isDarkMode ? 'bg-black text-white' : 'bg-white text-gray-900'
          }`}
      >
        {/* Animated Background Elements */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className={`absolute top-20 left-10 w-64 h-64 rounded-full blur-3xl animate-float ${isDarkMode ? 'bg-[#0045EF]/5' : 'bg-[#0045EF]/10'
            }`}></div>
          <div className={`absolute bottom-20 right-10 w-96 h-96 rounded-full blur-3xl animate-float-delayed ${isDarkMode ? 'bg-purple-500/5' : 'bg-purple-500/10'
            }`}></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="text-center mb-12 sm:mb-16">
            ...
            <p className={`text-base sm:text-lg max-w-2xl mx-auto px-4 ${isDarkMode ? 'text-white/70' : 'text-gray-600'
              }`}>
            ...
            {serviceContent.map((item, index) => {
              ...
              return (
                <SwiperSlide key={index}>
                  <div
                    className={`group rounded-2xl backdrop-blur-sm p-6 sm:p-8 flex flex-col transition-all duration-500 hover:scale-[1.02] hover:-translate-y-2 ${isDarkMode
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 ${isDarkMode
                    ...
                    {/* Title & Subtitle */}
                    <div className="mb-6">
                      <h3 className={`text-xl sm:text-2xl font-bold mb-2 group-hover:text-[#0045EF] transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-gray-900'
                        }`}>
                        {item.title}
                      </h3>
                    ...
                    <div className={`rounded-xl p-4 sm:p-6 mb-6 h-40 sm:h-48 overflow-hidden flex items-center justify-center transition-colors duration-300 ${isDarkMode
                      ...
                      {item.title === "Custom Software" ? (
                        ...
                        item.title === "Digital Transformation" ? (
                          <div className="w-full">
                            <p className={`text-xs mb-3 font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'
                              }`}>
                              System Upgrade in Progress...
                            </p>
                            <div className={`w-full h-4 rounded-full relative overflow-hidden shadow-inner ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'
                              }`}>
                            ...
                            item.title === "Digital Marketing" ? (
                              <div className={`w-full text-left ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                                ...
                                <div className="space-y-3">
                                  <div>
                                    <div className={`mb-1 text-xs flex justify-between ${isDarkMode ? 'text-gray-300' : 'text-gray-600'
                                      }`}>
                                      <span>Visitors</span>
                                      <span className="font-bold text-[#0045EF]">12.4k</span>
                                    </div>
                                    <div className={`w-full h-2 rounded-full relative overflow-hidden ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'
                                      }`}>
                                ...
                                <div className="w-full text-center">
                                  <div className="flex flex-col gap-3 items-center">
                                    <div className="flex gap-2 sm:gap-3 items-center">
                                      <span className={`px-3 py-2 rounded-lg text-xs font-semibold ${isDarkMode
                                    ...
                                    <p className={`text-xs italic mt-1 flex items-center gap-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'
                                      }`}>
                    ...
                    <div className="mb-6" style={{ minHeight: "60px" }}>
                      <p className={`text-sm sm:text-base leading-relaxed ${isDarkMode ? 'text-white/80' : 'text-gray-600'
                        }`}>
                        {item.desc}
                      </p>
                    </div>
          ...
          {/* Previous Button */}
          <div className={`swiper-button-prev-custom group absolute left-2 lg:left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 backdrop-blur-sm rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 -ml-4 sm:-ml-6 hover:bg-[#0045EF] ${isDarkMode
            ? 'bg-gray-800/50 border border-gray-700 hover:border-[#0045EF]'
            : 'bg-gray-200/50 border border-gray-300 hover:border-[#0045EF]'
            }`}>
            <ChevronLeft
              className={`w-6 h-6 transition-colors duration-300 group-hover:text-white ${isDarkMode ? 'text-white' : 'text-gray-900'
                }`}
              strokeWidth={2.5}
            />
          </div>
          ...
          {/* Next Button */}
          <div className={`swiper-button-next-custom group absolute right-2 lg:right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 backdrop-blur-sm rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 -mr-4 sm:-mr-6 hover:bg-[#0045EF] ${isDarkMode
            ? 'bg-gray-800/50 border border-gray-700 hover:border-[#0045EF]'
            : 'bg-gray-200/50 border border-gray-300 hover:border-[#0045EF]'
            }`}>
            <ChevronRight
