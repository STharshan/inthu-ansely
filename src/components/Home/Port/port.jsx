...
export default function ExpenseControlCenter() {
  ...
  return (
    <div className="bg-white h-1000 overflow-hidden overflow-y-hidden" >
      {/* Background gradient */}
      ...
      >
        ...
        <p className="text-xl  mt-5 mb-6 font-semibold text-[#3A7C7A] leading-tight max-w-xs sm:max-w-xl md:max-w-2xl lg:max-w-3xl px-4">
      ...
      "
            // Add an onClick handler if needed for functionality
          ...
          return (
            <ScrollStackItem key={feature.id}>
              ...
              >
                {/* Image */}
                <div
                  className={`relative w-full md:w-1/2 h-48 xs:h-56 sm:h-64 md:h-72 lg:h-80 xl:h-[30rem] rounded-xl sm:rounded-2xl overflow-hidden order-1 ${isImageLeft ? "md:order-1" : "md:order-2"
                    }`}
                >
                  <img
                    src={feature.image}
                ...
                {/* Content */}
                <div
                  className={`w-full md:w-1/2 flex flex-col justify-center text-center md:text-left order-2 ${isImageLeft ? "md:order-2" : "md:order-1"
                    }`}
                >
